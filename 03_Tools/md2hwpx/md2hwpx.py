...
def preprocess_markdown(text, number_headings=True):
    ...
    1. Strips existing heading numbers and applies hierarchical numbering (if number_headings=True)
...
def calc_col_widths(rows, total_width=TABLE_WIDTH):
    ...
    Special rule: if the last column header is '비고', fix it at 10% of total width.
    """
    if not rows:
        return []
    ...
    # 마지막 컬럼이 '비고'이면 10% 고정
    last_header = rows[0][-1].strip() if rows[0] else ''
    if last_header == '비고' and col_count > 1:
        fixed_last = int(total_width * 0.10)
        remaining = total_width - fixed_last
        total_len = sum(max_lens[:-1])
        widths = [int(remaining * l / total_len) for l in max_lens[:-1]]
        widths[-1] = remaining - sum(widths[:-1])  # 나머지 보정
        widths.append(fixed_last)
    else:
...
def build_section_xml(elements, title="", author=""):
    paras = []
    ...
    paras.append(
        ...
        f'{xml_escape(("문서 작성자: " + author) if author else "")}'
        f'</hp:t></hp:run></hp:p>'
    )
    ...
    for etype, data in elements:
        ...
        if etype == 'heading':
            ...
            paras.append(empty_para())  # 표 아래 빈 줄
...
def build_content_hpf(title="Untitled", author="", subject="", description="",
...
def convert(input_md, output_hwpx, skeleton_path=None, title=None, author="", number_headings=True,
    ...
    try:
        ...
        report = preprocess_markdown(report, number_headings=number_headings)
        ...
        section_xml = build_section_xml(elements, title=title, author=author)
...
def main():
    ...
    parser.add_argument('--no-numbering', action='store_true', help='Do not auto-number headings')
    ...
    convert(
        ...
        number_headings=not args.no_numbering,
